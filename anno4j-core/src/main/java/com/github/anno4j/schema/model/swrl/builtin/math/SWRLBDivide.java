 ...
 */
@SWRLBuiltinIri(SWRLB.DIVIDE)
public class SWRLBDivide extends SWRLBuiltin implements Computation {
     ...
     */
    public SWRLBDivide(List<Object> arguments) {
        super(arguments);

        // Check that there are three arguments:
        if(arguments.size() != 3) {
            throw new IllegalArgumentException("swrlb:divide expects three arguments. " + arguments.size() + " passed.");
        }
        // Check all non-variable arguments are numeric:
        for(Object argument : arguments) {
            if(!(argument instanceof Variable || argument instanceof Number)) {
                throw new IllegalArgumentException("swrlb:divide expects non-variable arguments to be numeric.");
            }
        }
    }
