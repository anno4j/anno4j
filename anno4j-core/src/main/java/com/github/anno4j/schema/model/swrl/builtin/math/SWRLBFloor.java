 ...
 * <a href="http://www.w3.org/2003/11/swrlb#floor">http://www.w3.org/2003/11/swrlb#floor</a> .
 * It receives two arguments x, y. The predicate is true iff x = floor(y) holds.
 * This built-in is SPARQL serializable.
 */
@SWRLBuiltinIri(SWRLB.FLOOR)
public class SWRLBFloor extends SWRLBuiltin implements SPARQLSerializable {
     ...
     */
    public SWRLBFloor(List<Object> arguments) {
        super(arguments);

        // Check that there are two arguments:
        if(arguments.size() != 2) {
            throw new IllegalArgumentException("swrlb:floor expects two arguments. " + arguments.size() + " passed.");
        }
        // Check all non-variable arguments are numeric:
        for(Object argument : arguments) {
            if(!(argument instanceof Variable || argument instanceof Number)) {
                throw new IllegalArgumentException("swrlb:floor expects non-variable arguments to be numeric.");
            }
        }
    }
    ...
    public boolean evaluate(Bindings bindings) {
        ...
        if(value1 instanceof Number && value2 instanceof Number) {
            return ((Number) value1).doubleValue() == Math.floor(((Number) value2).doubleValue());
        } else {
            return false;
        }
    }
