 ...
 * <a href="http://www.w3.org/2003/11/swrlb#greaterThan">http://www.w3.org/2003/11/swrlb#greaterThan</a> .
 * It receives two arguments x, y. The predicate is true iff x > y holds.
 * This built-in is SPARQL serializable.
 */
@SWRLBuiltinIri(SWRLB.GREATER_THAN)
public class SWRLBGreaterThan extends SWRLBuiltin implements SPARQLSerializable {
     ...
     */
    public SWRLBGreaterThan(List<Object> arguments) {
        super(arguments);

        if(arguments.size() != 2) {
            throw new IllegalArgumentException("swrlb:greaterThan expects two arguments. " + arguments.size() + " passed.");
        }
        for(Object argument : arguments) {
            if(!(argument instanceof Variable || argument instanceof Comparable)) {
                throw new IllegalArgumentException("Arguments of swrlb:greaterThan must implement java.lang.Comparable");
            }
        }
    }
