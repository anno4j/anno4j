 ...
 * <a href="http://www.w3.org/2003/11/swrlb#add">http://www.w3.org/2003/11/swrlb#add</a> .
 ...
 */
@SWRLBuiltinIri(SWRLB.ADD)
public class SWRLBAdd extends SWRLBuiltin implements Computation {
     ...
     */
    public SWRLBAdd(List<Object> arguments) {
        super(arguments);

        // Check that there are three arguments:
        if(arguments.size() != 3) {
            throw new IllegalArgumentException("swrlb:add expects three arguments. " + arguments.size() + " passed.");
        }
        // Check all non-variable arguments are numeric:
        for(Object argument : arguments) {
            if(!(argument instanceof Variable || argument instanceof Number)) {
                throw new IllegalArgumentException("swrlb:add expects non-variable arguments to be numeric.");
            }
        }
    }
