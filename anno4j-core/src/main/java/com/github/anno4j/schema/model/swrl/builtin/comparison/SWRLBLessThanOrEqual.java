 ...
 * <a href="http://www.w3.org/2003/11/swrlb#lessThanOrEqual">http://www.w3.org/2003/11/swrlb#lessThanOrEqual</a> .
 * It receives two arguments x, y. The predicate is true iff x <= y holds.
 * This built-in is SPARQL serializable.
 */
@SWRLBuiltinIri(SWRLB.LESS_THAN_OR_EQUAL)
public class SWRLBLessThanOrEqual extends SWRLBuiltin implements SPARQLSerializable {
     ...
     */
    public SWRLBLessThanOrEqual(List<Object> arguments) {
        super(arguments);

        if(arguments.size() != 2) {
            throw new IllegalArgumentException("swrlb:lessThanOrEqual expects two arguments. " + arguments.size() + " passed.");
        }
        for(Object argument : arguments) {
            if(!(argument instanceof Variable || argument instanceof Comparable)) {
                throw new IllegalArgumentException("Arguments of swrlb:lessThanOrEqual must implement java.lang.Comparable");
            }
        }
    }
