 ...
 * <a href="http://www.w3.org/2003/11/swrlb#startsWith">http://www.w3.org/2003/11/swrlb#startsWith</a> .
 * Satisfied iff the first argument starts with the second argument.
 * This built-in is SPARQL serializable.
 */
@SWRLBuiltinIri(SWRLB.STARTS_WITH)
public class SWRLBStartsWith extends SWRLBuiltin implements SPARQLSerializable {
     ...
     */
    public SWRLBStartsWith(List<Object> arguments) {
        super(arguments);

        // Check that there are three arguments:
        if(arguments.size() != 2) {
            throw new IllegalArgumentException("swrlb:startsWith expects two arguments. " + arguments.size() + " passed.");
        }
    }

    @Override
    public String asSPARQLFilterExpression() {
        return "STRSTARTS(" + getArgumentAsFilterExpression(0) + ", " + getArgumentAsFilterExpression(1) + ")";
    }
